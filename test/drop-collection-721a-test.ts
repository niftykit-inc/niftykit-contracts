...
import {
  createDropCollection721a,
  createNiftyKit,
  hashAccount,
} from "./utils/collections";

const salesParam = [100, 100, 100, ethers.utils.parseEther("0.01")] as const;

describe("DropCollection (721A)", function () {
  let accounts: Signer[];
  ...
  it("should be able to create a DropCollection (721A)", async function () {
    await createDropCollection721a(niftyKit, accounts[0]);
  });

  it("should be able to start the drop", async function () {
    const dropCollection = await createDropCollection721a(
      niftyKit,
      accounts[0]
    );
  ...
  it("should be able to mint", async function () {
    ...
    const txMint = await dropCollection.connect(accounts[1]).mint(50, {
      value: salesParam[3].mul(50),
    });
